...
public function showInternDetailsMain()
{
    ...
    $currentInternsCount = Intern::whereDate('startDate', '<=', $currentDate)
        ->where(function($query) use ($currentDate) {
            $query->whereDate('endDate', '>=', $currentDate)
                  ->orWhereNull('endDate');
        })
        ->count();

    $upcomingInternsCount = Intern::whereDate('startDate', '>', $currentDate)->count();
    ...
    return view('pages.dashboard.main', [
        ...
        'currentInternsCount' => $currentInternsCount,
        'upcomingInternsCount' => $upcomingInternsCount,
        'interns' => $interns,
    ]);
...
{
    $interns = Intern::all();
    return view('pages.dashboard.stagiaires', ['interns' => $interns]);
}
...
{
    // Validation, mise à jour des données, etc.
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'cinFile' => 'nullable|file|mimes:pdf,jpeg,png',
            'demande' => 'nullable|file|mimes:pdf',
            ...
            'supervisor.required' => 'L\'encadrant est requis.',
            'cv.required' => 'Le CV est requis.',
            'cv.mimes' => 'Le CV doit être un fichier PDF, DOC ou DOCX.',
            'cinFile.required' => 'Le fichier CIN est requis.',
            'cinFile.mimes' => 'Le fichier CIN doit être un PDF, JPEG ou PNG.',
            'demande.required' => 'La demande est requise.',
        ...
        if ($request->hasFile('cv')) {
            ...
            $cvPath = $request->file('cv')->store('public/files/cvs');
            $intern->cv = 'storage' . substr($cvPath, 6);
        }
        
        if ($request->hasFile('cinFile')) {
            Storage::delete($intern->cinFile);
            $cinFilePath = $request->file('cinFile')->store('public/files/cin');
            $intern->cinFile = 'storage' . substr($cinFilePath, 6);
        }
        
        if ($request->hasFile('demande')) {
            Storage::delete($intern->demande);
            $demandePath = $request->file('demande')->store('public/files/demandes');
            $intern->demande = 'storage' . substr($demandePath, 6);
        }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'cinFile' => 'required|file|mimes:pdf,jpeg,png',
            'demande' => 'required|file|mimes:pdf',
        ], [
            'firstName.required' => 'Le prénom est requis.',
            'lastName.required' => 'Le nom de famille est requis.',
            'age.required' => 'L\'âge est requis.',
        ...
        if ($intern->image !== "/images/intern.png") {
            // Suppression de l'ancienne image : enlever 'storage/' au début
            $oldImage = str_replace('storage/', '', $intern->image);
